	...
	void animate(void actionEvent()) {
		...
		if (isAnimating) {
			...
			if (xoff+x>=x+xelev) {
			  ...
			  xoff = max(0.0f, xoff - xelev / clickdelay);
    		  yoff = max(0.0f, yoff - yelev / clickdelay);
		}
	}
...
float button::clickdelay = max( 10.0, 1000 / (CUBE_SIZE * sqrt(CUBE_SIZE)));
...
double transitionSpeed = min(1.0, pow(10, floor((CUBE_SIZE - 3) / 10)) / 1000);
...
button bhelp = button(-3.5, 0.5, 0.8, 0.3, color(1, 0.8, 0), color(0, 0, 0), "HELP");
button bexit = button(-3.5, 0, 0.8, 0.3, color(0, 0.6, 1), color(0, 0, 0), "EXIT");

button bback10 = button( 4.5, -2, 0.8, 0.3, color(1, 0.1, 0.1), color(0, 0, 0), "BACK");
button bhollow = button( 4.5,  2, 0.8, 0.3, color(0.2, 0.2, 0.2), color(1, 1, 1), "BERONGGA");
button bsolve = button( 4.5, 1.5, 0.8, 0.3, color(1, 0, 0.6), color(0, 0, 0), "MEMECAHKAN");
...
color colorList[7] = { color(1,0,1), color(0,0.5,0.5), color(0.5,1,1), color(2,0.5,1),  color(0.81,0.71,0.23), color(0.9,0,0), color(0.2,0.2,0.2) };

Quaterniond cube1sRotation[CUBE_SIZE][CUBE_SIZE][CUBE_SIZE];
...
void toggleHollow() {
	...
	if (hollow) {
		hollow = false;
		bhollow = button(4.5, 2, 0.8, 0.3, color(0.2, 0.2, 0.2), color(1, 1, 1), "BERONGGA");
	}
	else  {
		hollow = true;
		bhollow = button(4.5, 2, 0.8, 0.3, color(0.9, 0.9, 0.9), color(0, 0, 0), "MENYATUKAN");
	}
}
...
void changeState() {
	...
	REP(a, rotationQueue.size()) {
		...
		cube1sRotation[i][j][k] = Quaterniond{ AngleAxisd{ 1, Vector3d{ 0,0,0 } } };
	}
...
void buildRubiksCube() {
	...
	double intercube1_spacing = small_size*0.05;
	if(CUBE_SIZE>20)
		intercube1_spacing = 0;

	double start = big_szie/2 + intercube1_spacing*(CUBE_SIZE-1)/2;

	for (double z = start; z>-start; z-= small_size+intercube1_spacing) {
		for (double y = start; y>-start; y-= small_size+intercube1_spacing) {
			for (double x = -start; x<start; x += small_size+intercube1_spacing) {

				int cx = (int)round((x+big_szie/2)/(small_size+intercube1_spacing));
				int cy = (int)round((-y+big_szie/2)/(small_size+intercube1_spacing));
				int cz = (int)round((-z+big_szie/2)/(small_size+intercube1_spacing));

				glPushMatrix();
				glTranslatef(8, 0, 0);
				glMultMatrixd(getRotationMatrix(cube1sRotation[cz][cy][cx]));
				glTranslatef(-8, 0, 0);
				float v[8][3] = { 
					{ x+8, y, z },
					{ x+small_size+8, y, z },
...
void updateRotation() {
	...
	REP(a, rotationQueue.size()) {
		...
		cube1sRotation[i][j][k] = qr*cube1sRotation[i][j][k];
	}
